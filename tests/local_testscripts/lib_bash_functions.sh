...
function cleanup() {
  ...
  rm -f  /usr/local/bin/keep_github_workflows_active
  cd "${save_dir}" || exit
...
function test_commandline_interface_venv() {
  ...
  clr_green "issuing command : /usr/local/bin/keep_github_workflows_active --version"
  if ! "/usr/local/bin/keep_github_workflows_active" --version; then
...
function test_commandline_interface_venv_old() {
  ...
  clr_green "issuing command : $HOME/venv/local/bin/keep_github_workflows_active --version"
  if ! "$HOME/venv/local/bin/keep_github_workflows_active" --version; then
    my_banner_warning "test commandline interface on virtual environment ERROR"
    beep
    sleep "${sleeptime_on_error}"
    return 1
